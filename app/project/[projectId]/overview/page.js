...
import { activities as initialActivities, meeting, project, tasks as initialTasks } from "@/data/mockData";
import { calculateProgress, getProjectActivities, getProjectTasks } from "@/lib/projectState";
import { clearProjectAccess, getProjectAccessCode } from "@/lib/projectAccess";
...
export default function ProjectOverviewPage() {
  ...
  const [projectActivities, setProjectActivities] = useState(initialActivities);
  useEffect(() => {
    const sync = () => { setProjectTasks(getProjectTasks(project.id)); setProjectActivities(getProjectActivities(project.id)); };
    sync();
    window.addEventListener("aipc-project-state", sync);
    window.addEventListener("storage", sync);
    return () => { window.removeEventListener("aipc-project-state", sync); window.removeEventListener("storage", sync); };
  }, []);
  const overallProgress = calculateProgress(projectTasks);
  ...
  return <PinVerifiedGuard><div className="min-h-screen bg-slate-50 text-slate-950 dark:bg-zinc-950 dark:text-white">
    ...
    {accessOpen && <Modal title="Project Access" description="Shared access details for this project." onClose={()=>setAccessOpen(false)}><div className="p-6"><span className="text-xs font-semibold text-slate-500">Project Access Code</span><div className="mt-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-xl font-bold tracking-[.35em] dark:border-white/10 dark:bg-zinc-950">{accessCode}</div><div className="mt-5 space-y-3 text-sm leading-6 text-slate-500"><p>This is the permanent access code for this project.</p><p>All members of this project use this same access code to join.</p><p>Share this code only with project members. This code cannot be changed.</p></div><div className="mt-6 flex flex-wrap justify-end gap-2"><button onClick={()=>navigator.clipboard?.writeText(accessCode)} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold dark:border-white/10">Copy Code</button><button onClick={()=>setAccessOpen(false)} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold hover:bg-slate-50 dark:border-white/10 dark:hover:bg-white/5">Close</button></div></div></Modal>}
