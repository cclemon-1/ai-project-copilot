...
import { activities, members, meeting, project, tasks } from "@/data/mockData";
import { clearProjectAccess } from "@/lib/projectAccess";
...
export default function ProjectOverviewPage() {
  ...
  return <PinVerifiedGuard><div className="min-h-screen bg-slate-50 text-slate-950 dark:bg-zinc-950 dark:text-white">
    ...
    <main className="mx-auto max-w-6xl px-5 py-10">
      <section className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><span className="text-xs font-bold tracking-widest text-blue-600">PROJECT OVERVIEW</span><h1 className="mt-2 text-3xl font-bold tracking-tight">{project.name}</h1><p className="mt-2 text-sm text-slate-500">{project.workspace} · {project.course} · {project.university}</p></div><button onClick={()=>setAccessOpen(true)} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-600 shadow-sm hover:border-blue-300 hover:text-blue-700 dark:border-white/10 dark:bg-zinc-900 dark:text-zinc-300">🔒 Project Access</button></section>
      <div className="mt-7 grid gap-4 lg:grid-cols-[1.4fr_.8fr]"><article className={card}><div className="flex items-center justify-between"><span className="text-xs font-bold text-violet-600">✦ AI PROJECT SUMMARY</span><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">● {project.status}</span></div><p className="mt-4 text-sm leading-6 text-slate-500">{project.summary}</p></article><article className={card}><span className="text-xs font-bold tracking-widest text-blue-600">TEAM</span><div className="mt-4 flex -space-x-2">{members.map(([initials,name,,color])=><span title={name} key={name} className={`grid size-10 place-items-center rounded-full border-2 border-white text-[10px] font-bold text-white dark:border-zinc-900 ${color}`}>{initials}</span>)}</div><p className="mt-3 text-xs text-slate-500">6 members · Shared project workspace</p></article></div>
    ...
    {accessOpen && <Modal title="Project Access" description="Shared access details for this project." onClose={()=>setAccessOpen(false)}><div className="p-6"><span className="text-xs font-semibold text-slate-500">Current Project PIN</span><div className="mt-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-xl font-bold tracking-[.35em] dark:border-white/10 dark:bg-zinc-950">205205</div><div className="mt-5 space-y-3 text-sm leading-6 text-slate-500"><p>This project is protected by a shared six-digit PIN.</p><p>The Project Leader shares the PIN with team members when the project is created.</p><p>Only the Project Leader and Vice Leader can update the Project PIN.</p></div><div className="mt-6 flex justify-end"><button onClick={()=>setAccessOpen(false)} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold hover:bg-slate-50 dark:border-white/10 dark:hover:bg-white/5">Close</button></div></div></Modal>}
