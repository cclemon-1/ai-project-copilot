...
export default function ProjectOverviewPage() {
  ...
  return <PinVerifiedGuard><div className="min-h-screen bg-slate-50 text-slate-950 dark:bg-zinc-950 dark:text-white">
    ...
    <main className="mx-auto max-w-6xl px-5 py-10">
      <section className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><span className="text-xs font-bold tracking-widest text-blue-600">PROJECT OVERVIEW</span><h1 className="mt-2 text-3xl font-bold tracking-tight">{project.name}</h1><p className="mt-2 text-sm text-slate-500">{project.workspace} · {project.course} · {project.university}</p></div><button onClick={()=>router.push(`/project/${project.id}/select-member`)} className="rounded-xl bg-blue-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-600/20">Open Dashboard →</button></section>
