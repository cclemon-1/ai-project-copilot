...
import { canEditProject, getSelectedMember } from "@/lib/projectAccess";
    ...
    <AccessGuard><div className="min-h-screen bg-slate-50 text-slate-950 dark:bg-zinc-950 dark:text-zinc-50">
      <div className="grid min-h-screen md:grid-cols-[220px_1fr]">
        <aside className="hidden border-r border-slate-200 bg-white p-4 md:flex md:flex-col dark:border-white/10 dark:bg-zinc-900">
          ...
          {canEdit ? <Link href={`/project/${project.id}/settings`} className="mt-auto rounded-lg px-3 py-2.5 text-xs text-slate-600 hover:bg-slate-100 dark:text-zinc-400 dark:hover:bg-white/5">⚙ Project Settings</Link> : <span className="mt-auto px-3 py-2 text-[10px] text-slate-400">Signed in as {member?.name || "member"}</span>}
        </aside>
